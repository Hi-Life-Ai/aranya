...
const Poscreate = () => {
    ...
    //     onBeforeGetContent: () => {
    //         const copies = [
    //             { title: 'Duplicate Copy', data: invoiceData },
    //             { title: 'Customer Copy', data: invoiceData }
    //         ];
    ...
    // const handlePrint = useReactToPrint({
    //     content: () => componentRef.current,
    //     documentTitle: 'Aranya Health Care',
    //     pageStyle: 'print',
    //     onAfterPrint: () => {
    //         if (!copiesRendered) {
    //             const paginatedData = paginateTableData(invoiceData.invoiceTableData, 10); // Adjust the number of products per page as needed

    //             // Render each copy with paginated data

    //             for (const pageData of paginatedData) {
    //                 renderAndAppendInvoiceCopy({ ...invoiceData, invoiceTableData: pageData }, 'Duplicate Copy');
    //             }
    //             for (const pageData of paginatedData) {
    //                 renderAndAppendInvoiceCopy({ ...invoiceData, invoiceTableData: pageData }, 'Customer Copy');
    //             }
    ...
    const renderAndAppendInvoiceCopy = (invoiceData, copyTitle, copyType) => {
        const copyContainer = document.createElement('div');
        copyContainer.style.pageBreakAfter = 'always';
        ...
        // Render the invoice layout of InvoiceComponent
        ReactDOM.render(
            <InvoiceComponent data={invoiceData} title={copyTitle} copyType={copyType} />,
            copyContainer
        );

        // Add a delay before appending to ensure rendering is complete
        setTimeout(() => {
            componentRef.current.appendChild(copyContainer);
    ...
    const handlePrint = useReactToPrint({
        ...
        onAfterPrint: () => {
            if (!copiesRendered) {
                ...
                // Generate Original, Duplicate, and Customer copies for each page
                for (const pageData of paginatedData) {
                    renderAndAppendInvoiceCopy(
                        { ...invoiceData, invoiceTableData: pageData },
                        'Original Copy',
                        'Original'
                    );
                    renderAndAppendInvoiceCopy(
    ...
    const handlePosSubmit = (e) => {
        ...
        if (tableData.length == 0) {
            ...
            //     renderAndAppendInvoiceCopy(invoiceData, 'Duplicate Copy');

            //     // Render and append the third copy (InvoiceComponent layout)
            //     renderAndAppendInvoiceCopy(invoiceData, 'Customer Copy');
                    ...
                    <Box >
                        <Grid container >
                            ...
                            <Grid item lg={6} md={6} sm={6} xs={6} sx={{ textAlign: 'right', }}>
                                <Typography sx={{ fontSize: '12px', fontWeight: "1000", }}>Original Copy</Typography>
                                <Grid container>
