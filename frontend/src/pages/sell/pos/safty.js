    ...
    const handlePrint = useReactToPrint({
        ...
        onBeforeGetContent: () => {
            ...
            for (const copy of copies) {
                ...
                for (let i = 0; i < Math.ceil(copy.data.tableData.length / 10); i++) {
                    paginatedTableData.push(copy.data.tableData.slice(i * 10, (i + 1) * 10));
                }

                for (const chunk of paginatedTableData) {
                    const chunkContainer = document.createElement('div');
                    chunkContainer.style.pageBreakAfter = 'always';

                    // Render the paginated tableData directly
                    ReactDOM.render(
                        <Box>
                            {/* Your existing rendering logic */}
                            {chunk.map((data, i) => (
                                <TableRow key={i} sx={{ paddingTop: '5px' }}>
                    ...
                    componentRef.current.appendChild(chunkContainer);
                }
            }
        }
    });
