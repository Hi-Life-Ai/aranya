...
function MonthwiseprofitList() {
    ...
    const fetchMatchLocationandMonth = async () => {
        ...
        try {
            ...
            req.data.pos1.map(item => {
                var mm = new Date(item.date).getMonth() + 1
                if (dateFilter.location == item.location && mm == dateFilter.month) {
