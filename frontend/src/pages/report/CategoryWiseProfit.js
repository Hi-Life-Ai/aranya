...
function CategoryProfit() {
  ...
  const fetchCategory = async () => {
    try {
      ...
      setCateOptions(request.data.result.map((data) => ({
        ...data,
  ...
  const fetchCateProfit = async (e) => {

    try {
      let req_data = await axios.post(SERVICE.CATE_WISE_PROFIT, {
        headers: {
          'Authorization': `Bearer ${auth.APIToken}`
        },
        businessid: String(setngs.businessid),
        ...
        category: String(e.categoryname)
