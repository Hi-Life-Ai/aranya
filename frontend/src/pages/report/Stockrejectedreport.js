...
function Stockrejectreportlist() {
    ...
    // Excel
    const fileName = 'Stock Reject Report '
    // get particular columns for export excel
    const getexcelDatas = async () => {
        var data = stockReject.map(t => (
            {
                'Date': t.date,
    ...
    const handleprint = useReactToPrint({
        ...
        documentTitle: 'ARANYA HERBALS | STOCK REJECT REPORT ',
        pageStyle: 'print'
    });
    ...
    const downloadPdf = () => {
        ...
        doc.save('Stock Reject Report .pdf')
    }
    ...
    const sortedData = stockReject.sort((a, b) => {
    ...
    const filteredDatas = stockReject?.filter((item) =>
    ...
    const totalPages = Math.ceil(stockReject.length / pageSize);
    ...
    return (
        <Box >
            <Headtitle title={'Stock Reject Report '} />
            { /* ****** Header Content ****** */}
            <Typography sx={userStyle.HeaderText}>Stock Reject Report </Typography>
            { /* ****** Table Start ****** */}
            <>
                ...
                <Box sx={userStyle.container} >
                    <Grid style={userStyle.dataTablestyle}>
                        <Box>
                            ...
                            <Select id="pageSizeSelect" value={pageSize} onChange={handlePageSizeChange} sx={{ width: "77px" }}>
                                ...
                                <MenuItem value={(stockReject.length)}>All</MenuItem>
                            </Select>
                    ...
                    <Grid container sx={{ justifyContent: "center" }} >
                        <Grid>
                            {isUserRoleCompare[0].printstockrejectedreport && (
                                <>
                                    <Button sx={userStyle.buttongrp} onClick={handleprint}>&ensp;<FaPrint />&ensp;Print&ensp;</Button>
                                </>
                            )}
                            {isUserRoleCompare[0].pdfstockrejectedreport && (
                                <>
                    ...
                    <Box>
                        ...
                        <Box style={userStyle.dataTablestyle}>
                            <Box>
                                Showing {((page - 1) * pageSize) + 1} to {Math.min(page * pageSize, stockReject.length)} of {stockReject.length} entries
                            </Box>
                            <Box>
                    ...
                    <Box  >
                        <TableContainer component={Paper} sx={userStyle.printcls}>
                            <Table aria-label="simple table" id="pdftable" ref={componentRef}>
                                ...
                                <TableBody>
                                    {stockReject.length > 0 ?
                                        (stockReject.map((row, index) => (
                                            <StyledTableRow key={index}>
                                                <StyledTableCell align="left">{row.date}</StyledTableCell>
                                                <StyledTableCell align="left">{row.fromlocation + ", "}</StyledTableCell>
                                                <StyledTableCell align="left">{row.products.map((value) => value.locations + ", ")}</StyledTableCell>
...
function Stockrejectreport() {
    return (
        <Box>
            <Navbar />
            <Box sx={{ width: '100%', overflowX: 'hidden' }}>
                <Box component="main" sx={{ paddingRight: '60px', paddingLeft: '60px', paddingTop: '20px', '@media (maxWidth: 600px)': { paddingLeft: '30px', paddingRight: '30px' } }}>
                    <Stockrejectreportlist /><br /><br /><br /><br />
                    <Footer />
                </Box>
            </Box>
        </Box>
...
export default Stockrejectreport;